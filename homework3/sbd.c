...
/* Assignment 3 */
static char *encryption_key = "mihaidan";
...
static void sbd_transfer(struct sbd_device *dev, sector_t sector, unsigned long nsect, char *buffer, int write) {
	...
	/* Assignment 3: Adding encryption before writing and
	   decryption before reading. */
    if(write){
		source = buffer;
		destination = dev->data + offset;
		
		/* Encrypt the data and store it in the destination (block device) */
	...
	// else it's a read request
		...
		/* Decrypt the data and store it in the destination (buffer) */
		for(i = 0; i < nbytes; i += crypto_cipher_blocksize(cipher)){
